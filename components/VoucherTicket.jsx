...
const VoucherTicket = ({ item, currency, small }) => {
  const statusMap = {
    USED: { label: "Đã dùng", color: "bg-gray-300 text-gray-600" },
    EXPIRED: { label: "Hết hạn", color: "bg-red-200 text-red-700" },
    DEFAULT: { label: "Chưa dùng", color: "bg-green-100 text-green-800" },
  };
    ...
    <div className={`relative flex flex-col md:flex-row items-stretch bg-white rounded-xl shadow-md overflow-hidden border border-dashed border-orange-300 ${minH}`}>
      ...
      <div className={`flex-1 flex flex-col justify-between ${padding} gap-1`}> 
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500">
          <span>
            Hiệu lực: {formatDate(item.couponId?.startDate)} - {formatDate(item.couponId?.endDate)}
          </span>
