      ...
      {user ? (
        <form onSubmit={editingComment ? (e) => { e.preventDefault(); handleUpdate(editingComment._id); } : handleSubmit} className="mb-6">
            ...
            <div className="mb-3 p-2 bg-orange-50 rounded-lg">
              <div className="flex items-center justify-between">
                <span className="text-sm text-orange-600">
                  {editingComment ? "Sửa bình luận" : `Trả lời ${replyingTo?.userId?.name}`}
                </span>
                <button
                  type="button"
          ...
          <textarea
            ...
            placeholder={replyingTo ? "Nhập trả lời..." : "Nhập bình luận..."}
            rows={3}
          />
          <button
          ...
          >
            {editingComment ? "Sửa" : replyingTo ? "Trả lời" : "Bình luận"}
          </button>
        </form>
      ) : (
        <p className="text-center text-gray-500 mb-6">Vui lòng đăng nhập để bình luận.</p>
      )}
