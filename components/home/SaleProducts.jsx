...
const SaleProducts = ({ initialData = null }) => {
  ...
  const handleToggleShow = () => {
    ...
    setTimeout(() => {
      if (newShowAll) {
        const buttonElement = document.querySelector('#sale-toggle-button');
        if (buttonElement) {
          buttonElement.scrollIntoView({ behavior: 'smooth', block: 'end' });
        }
      } else {
        const titleElement = document.querySelector('#sale-products-title');
        if (titleElement) {
  ...
  return (
    <>
      <div className="mt-12">
        <div className="flex flex-col items-start mb-8" id="sale-products-title">
